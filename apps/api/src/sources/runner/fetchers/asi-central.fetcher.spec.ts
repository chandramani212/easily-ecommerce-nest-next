...
describe('AsiCentralFetcher pagination', () => {
  ...
  it('collectCategoryProductIds lists all ids in a category past the 1000-cap, no detail fetch', async () => {
    // 1500 products in category PENS, product k priced at k. The mock honours
    // both `category:PENS` and `price:[lo to hi]`, and — like ASI — never
    // returns more than 1000 rows for one query. Only correct price bisection
    // under the category selection recovers all 1500.
    const CAP = 1000;
    const N = 1500;
    fetchMock = jest.fn(async (url: unknown) => {
      const u = String(url);
      if (u.includes('/products/search.json')) {
        ...
        if (!/category:PENS/.test(q)) {
          // outside the category selection: nothing
      ...
      // any detail fetch would be a bug for this method
      const m = u.match(/products\/(\d+)\.json/);
      return jsonResponse({ Id: Number(m![1]), Name: `P${m![1]}` });
    });
    global.fetch = fetchMock as unknown as typeof fetch;
    ...
    expect(ids).toEqual(range(1, N).map((r) => r.Id));
    // ids only — no product detail request was made
