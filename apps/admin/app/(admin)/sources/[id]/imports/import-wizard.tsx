...
/* ---- Mapping shape (mirrors apps/api/src/sources/runner/mapping.types.ts). */
...
export function ImportWizard({
  ...
  const [format, setFormat] = useState<SourceImportFormat>(
    imp?.format ?? "JSON",
  );
  ...
  useEffect(() => {
    ...
    void clientApi<{ valid: boolean; next: string[] }>(
      `/sources/cron-preview?expression=${encodeURIComponent(cron)}&count=3`,
    )
      .then((r) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit
        ? `/sources/${sourceId}/imports/${imp.id}`
        : `/sources/${sourceId}/imports`;
      const result = await clientApi<SourceImport>(url, {
        method: isEdit ? "PATCH" : "POST",
        body: JSON.stringify(payload),
      });
      router.push(`/sources/${sourceId}/imports/${result.id}`);
      router.refresh();
    } catch (err) {
  ...
  async function handleSample() {
    ...
    try {
      ...
      const result = await clientApi<{
        ...
        paths: string[];
      }>(`/sources/${sourceId}/imports/${imp.id}/sample`, {
        method: "POST",
        body,
      });
  ...
  async function handleDryRun() {
    ...
    try {
      ...
      const result = await clientApi(
        `/sources/${sourceId}/imports/${imp.id}/dry-run`,
        { method: "POST", body },
      );
      setDryRun(result);
    } catch (err) {
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Format *">
            <select
              value={format}
              onChange={(e) =>
                setFormat(e.target.value as SourceImportFormat)
              }
              className={inputCls}
            >
      ...
      >
        <p className="mb-3 text-xs text-[var(--admin-fg)]/60">
          Map each product field on the left to a path in the source record on
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_240px]">
          <div className="space-y-3">
                ...
                <>
                  ...
                  <div className="rounded-md border border-dashed border-[var(--admin-border)] p-2">
                    ...
                    <p className="mb-2 text-[11px] text-[var(--admin-fg)]/60">
                      With hierarchy mode on, source categories are recorded
                      in the Source Categories page and the storefront only
            ...
            <SubMapper
              ...
              hint="Aggregator feeds only: each record's supplier company. Captured as a Supplier and linked to the product."
              enabled={!!mapping.vendor}
              onToggle={(on) =>
                setMapping({
                  ...mapping,
                  vendor: on
                    ? mapping.vendor ?? { name: { path: "" } }
                    : undefined,
                })
              }
            >
              {mapping.vendor && (
                <div className="space-y-2">
                  {(
                    [
                      ["name", "Supplier name *", "Vendor.Name path"],
                      ["externalId", "Vendor ID", "Vendor.Id path (else name)"],
                      ["phone", "Phone", "Vendor.Phone path"],
                      ["altPhone", "Alternate phone", "Vendor.Phone2 path"],
                      ["tollFree", "Toll-free", "Vendor.TollFree path"],
                      ["website", "Website", "Vendor.Url path"],
                    ] as const
                  ).map(([key, label, placeholder]) => (
                    <Field key={key} label={label}>
                      <SimpleFieldEditor
                        value={mapping.vendor![key] ?? { path: "" }}
                        onChange={(next) =>
                          setMapping({
                            ...mapping,
                            vendor: { ...mapping.vendor!, [key]: next },
                          })
                        }
                        placeholder={placeholder}
                      />
