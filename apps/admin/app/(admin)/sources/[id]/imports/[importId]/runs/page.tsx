...
export async function generateStaticParams() {
  ...
  return (mod.mockSourceImports ?? []).map((imp) => ({
    id: imp.sourceId,
    importId: imp.id,
  }));
}
...
export default async function RunHistoryPage({
  ...
  const [imp, runs] = await Promise.all([
    apiFetch<SourceImport>(`/sources/${id}/imports/${importId}`),
    apiFetch<{ items: SourceImportRun[]; total: number }>(
      `/sources/${id}/imports/${importId}/runs?take=${pageSize}&skip=${skip}`,
    ),
  ]);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      <PageHeader
        ...
        actions={
          <Link
            href={`/sources/${id}/imports/${importId}`}
