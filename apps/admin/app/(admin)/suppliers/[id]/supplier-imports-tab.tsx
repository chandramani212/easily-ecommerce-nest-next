...
import type { SupplierImportSummary } from "../../../../lib/types";
...
export function SupplierImportsTab({
  supplierId,
  imports,
}: {
  supplierId: string;
  ...
  async function runNow(importId: string) {
    ...
    setBusy(importId);
    try {
      await clientApi(`/suppliers/${supplierId}/imports/${importId}/run`, {
        method: "POST",
        body: JSON.stringify({}),
      });
      router.refresh();
    ...
    return (
      <div className="rounded-xl border border-dashed border-[var(--admin-border)] p-10 text-center">
        <p className="text-sm text-[var(--admin-fg)]/70">
          No imports configured for this supplier yet.
        </p>
        <Link
          href={`/suppliers/${supplierId}/imports/new`}
            ...
            {imports.map((imp) => (
              ...
              >
                <td className="px-4 py-3 font-medium">
                  <Link
                    href={`/suppliers/${supplierId}/imports/${imp.id}`}
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    <Link
                      href={`/suppliers/${supplierId}/imports/${imp.id}/runs`}
                      className="rounded-md border border-[var(--admin-border)] px-2.5 py-1 text-xs font-medium hover:bg-[var(--admin-muted)]"
                    >
                      Runs
                    </Link>
