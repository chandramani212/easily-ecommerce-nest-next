...
export function SupplierOverview({ supplier }: { supplier: Supplier }) {
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      <Card label="Imports" value={String(supplier.importCount ?? 0)} />
      <Card label="Linked products" value={String(supplier.productCount ?? 0)} />
      <Card
        label="Status"
      ...
      <div className="lg:col-span-3 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-5">
        ...
        <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
          <Row label="Source type" value={KIND_LABEL[supplier.kind] ?? supplier.kind} />
          <Row label="Auth type" value={AUTH_LABEL[supplier.authType] ?? supplier.authType} />
          <Row label="Base URL" value={supplier.baseUrl ?? "—"} mono />
          <Row
            label="Auth configured"
            value={supplier.authConfigured ? "Yes" : "No"}
          />
          <Row
            label="Default markup"
            value={
              supplier.defaultMarkupPct > 0
                ? `${supplier.defaultMarkupPct.toFixed(2)}%`
                : "—"
            }
          />
          <Row label="Created" value={formatDateTime(supplier.createdAt)} />
        </dl>
        {supplier.notes && (
