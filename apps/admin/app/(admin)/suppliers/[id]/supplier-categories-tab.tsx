...
export function SupplierCategoriesTab({ supplierId }: { supplierId: string }) {
    ...
    async function load() {
      ...
      try {
        ...
        const [list, cats] = await Promise.all([
          clientApi<{ total: number; items: SupplierCategoryRow[] }>(
            `/suppliers/${supplierId}/supplier-categories?${params.toString()}`,
          ),
  ...
  async function setMapping(rowId: string, categoryId: string | null) {
    ...
    try {
      const updated = await clientApi<{
        ...
        category: { id: string; name: string; slug: string } | null;
      }>(`/suppliers/${supplierId}/supplier-categories/${rowId}`, {
        method: "PATCH",
      ...
      <div className="rounded-xl border border-[var(--admin-border)]">
        <table className="w-full border-collapse text-sm">
          <thead className="bg-[var(--admin-muted)] text-left text-xs uppercase text-[var(--admin-fg)]/60">
            <tr>
              <th className="rounded-tl-xl px-3 py-2">Supplier category</th>
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={4} className="px-3 py-6 text-center text-[var(--admin-fg)]/60">
                  ...
                  enabled and supplier categories will show up.
                </td>
              </tr>
            )}
            {!loading &&
